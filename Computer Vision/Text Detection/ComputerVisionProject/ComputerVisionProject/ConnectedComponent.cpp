...
ConnectedComponent::ConnectedComponent(int max_component, int connectivity_type):maxComponent(max_component), connectivityType(connectivity_type){}
...
Mat ConnectedComponent::apply(const Mat& image) 
{
    ...
    if( connectivityType == 8 )
        func_ptr = bind(&ConnectedComponent::get8Neighbors, this, placeholders::_1, placeholders::_2, placeholders::_3);
    else
        func_ptr = bind(&ConnectedComponent::get4Neighbors, this, placeholders::_1, placeholders::_2, placeholders::_3);
    ...
    if(!temp.empty()) 
	{
        ...
        unique_copy(temp.begin(), temp.end(), back_inserter(labels));
    }
    ...
    sort
	(properties.begin(), properties.end(), [=](ComponentProperty& a, ComponentProperty& b)
		{
...
vector<int> ConnectedComponent::get4Neighbors(int * curr_ptr, int * prev_ptr, int x) 
{
    ...
    if( !neighbors.empty()) 
	{
        ...
        unique_copy(neighbors.begin(), neighbors.end(), back_inserter( result ));
    }
