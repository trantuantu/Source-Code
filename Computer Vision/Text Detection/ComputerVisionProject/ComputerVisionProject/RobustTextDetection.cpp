...
/* Rewrite round function cause VS2012 (I'm using it) isn't fully supported */
...
{
	/** Range of the detected MSER area, 
	  * for detected regions that are outside of those number of pixels, it's discarded 
	 **/
	P.minMSERArea         = 10;
    P.maxMSERArea         = 2000;
	...
	/**
	  ...
	  * Similar to the MSER params above, 
	  * any connected component above or below the range, will be discarded
	 **/
    P.maxConnCompCount    = 3000;
    P.minConnCompArea     = 75;
    P.maxConnCompArea     = 600;

    /**
	  * Detected region is a blob, or twisted, it's solid, and how spread out it is
	 **/
    P.minEccentricity    = 0.1;
    P.maxEccentricity    = 0.995;
    P.minSolidity        = 0.4;
    P.maxStdDevMeanRatio = 0.5;
}
...
pair<Mat, Rect> RobustTextDetection::apply(Mat &image) 
{
    ...
    for(ComponentProperty& prop: props) 
	{
        ...
        copy_if
		(
			vec.begin(), vec.end(), back_inserter(nonzero_vec), [&](int val)
			{
				return val > 0;
			}
		);
        ...
        double mean = accumulate(nonzero_vec.begin(), nonzero_vec.end(), 0.0) / area;
