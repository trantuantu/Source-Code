function [VW, sptHist] = LMdenseVisualWords(D, HOMEIMAGES_TRAIN, HOMEIMAGES_TEST, VWparam)
%
    ...
    if todo==1
        ...
        try
            switch typeD
                case 1
                   try
                        img = LMimread(D, n, HOMEIMAGES_TRAIN);
                   catch 'MATLAB:imagesci:imread:fileDoesNotExist'
                        img = LMimread(D, n, HOMEIMAGES_TEST);
                   end
                case 2
                    try
                        img = imread(fullfile(HOMEIMAGES_TRAIN, D{n}));
                    catch 'MATLAB:imagesci:imread:fileDoesNotExist'
                        img = imread(fullfile(HOMEIMAGES_TEST, D{n}));
                    end
                case 3
                    img = D(:,:,:,n);
            end
        ...
        switch VWparam.descriptor
            case 'sift'
                v = LMdenseSift(img, HOMEIMAGES_TRAIN, HOMEIMAGES_TEST, VWparam);
            case 'hog'
