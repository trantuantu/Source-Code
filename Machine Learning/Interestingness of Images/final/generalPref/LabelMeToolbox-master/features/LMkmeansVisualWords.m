function VWparam = LMkmeansVisualWords(D, HOMEIMAGES_TRAIN, HOMEIMAGES_TEST, VWparam)
%
...
for i = 1:Nimages
    ...
    try
        switch typeD
            case 1
                try
                    img = LMimread(D, i, HOMEIMAGES_TRAIN);
                catch 'MATLAB:imagesci:imread:fileDoesNotExist'
                    img = LMimread(D, i, HOMEIMAGES_TEST);
                end
            case 2
                try
                    img = imread(fullfile(HOMEIMAGES_TRAIN, D{i}));
                catch 'MATLAB:imagesci:imread:fileDoesNotExist'
                    img = imread(fullfile(HOMEIMAGES_TEST, D{i}));
                end
            case 3
                img = D(:,:,:,i);
        end
