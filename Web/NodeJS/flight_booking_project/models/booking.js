...
function Booking() {
  this.getBookingInfo = function(req, res) {
    ...
    connection.acquire(function(err, con) {
      con.query('select * from chi_tiet_chuyen_bay where MaDatCho = ?', req.id, function(err, result) {
        con.release();
        if (err)
          console.log(err);
        res.send(result);
 ...
 this.getBookingInfoByFlight = function(req, res) {
    console.log("Hello");
    connection.acquire(function(err, con) {
      con.query('select * from chi_tiet_chuyen_bay where MaChuyenBay = ? and Ngay =?', [req.flight, req.date], function(err, result) {
        con.release();
        if (err)
          console.log(err);
        res.send(result);
